...
export function MediaCard({ media, library, compact = false }) {
  ...
  const toggleSaved = () => {
    ...
    feedbackTimer.current = window.setTimeout(() => { setSaveFeedback(''); setIsSaving(false); setIsFavoriting(false) }, 1800)
  }
  const toggleFavorite = () => {
    const willFavorite = !saved?.favorite
    library.toggle(normalized, 'favorite')
    setSaveFeedback(willFavorite ? 'Adicionado aos favoritos' : 'Removido dos favoritos')
  ...
  return <article className="group min-w-0">
    ...
    <div className="pt-3">
      ...
      {!compact && <div className="mt-3"><div className="flex gap-2"><button className={`icon-button ${saved?.saved ? 'bg-emerald text-canvas hover:bg-emerald/90' : ''} ${isSaving ? 'animate-save-feedback' : ''}`} aria-label={`${saved?.saved ? 'Remover' : 'Adicionar'} ${normalized.title} ${saved?.saved ? 'da' : 'à'} lista`} aria-pressed={Boolean(saved?.saved)} onClick={toggleSaved}>{saved?.saved ? <Check size={18} aria-hidden="true" /> : <Plus size={18} aria-hidden="true" />}</button><button className={`icon-button ${saved?.favorite ? 'bg-red-400 text-canvas hover:bg-red-400/90' : ''} ${isFavoriting ? 'animate-save-feedback' : ''}`} aria-label={`${saved?.favorite ? 'Remover' : 'Adicionar'} ${normalized.title} ${saved?.favorite ? 'dos' : 'aos'} favoritos`} aria-pressed={Boolean(saved?.favorite)} onClick={toggleFavorite}><Heart size={18} fill={saved?.favorite ? 'currentColor' : 'none'} aria-hidden="true" /></button></div><p className={`mt-2 min-h-5 text-xs font-semibold ${feedbackTone === 'removal' ? 'text-red-400' : 'text-emerald'}`} role="status" aria-live="polite">{saveFeedback}</p></div>}
    </div>
  </article>
}
